...
class QuizzController extends Controller
{
    ...
    public function store(Request $request)
    {
        try {
            ...
            $quizzes->date_of_quiz = $request->date_of_quiz;
            $quizzes->quiz_duration = $request->quiz_duration;
            $quizzes->save();

            $users = Student::where('section_id', $quizzes->section_id)->get();

            foreach ($users as $user) {
                Notification::create([
                    'notification' => 'You have a new Quiz',
                    'seen_at' => null,
                    'model' => 'App\Models\Quizze',
                    'type' => 'new',
                    'destination_id' => $quizzes->id,
                    'student_id' => $user->id
                ]);
            }
    ...
    public function update(Request $request)
    {
        try {
            ...
            $quizz->date_of_quiz = $request->date_of_quiz;
            $quizz->quiz_duration = $request->quiz_duration;
            $quizz->save();
